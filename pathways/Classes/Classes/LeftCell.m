...
- (id)initWithSegs:(NSMutableArray *)segz andPt:(point *) pointt  andPos:(double) position
{
	...
	if (self) {
		...
		piece1 = [[LeftUnMovable alloc] initWithFrame:CGRectMake(X0, Y0, 2.3*(X1-X0), 2.3*(Y1-Y0))
									  andSegs:segs
										andPt:pt
									 andColor:[UIColor colorWithRed:1 green:1 blue:1 alpha:1]];
		piece2 = [[LeftMovable alloc] initWithFrame:CGRectMake(200, pos, 1.3*(X1-X0), 1.3*(Y1-Y0))
									  andSegs:segs
										andPt:pt
											andNum:1
										  andFrame:CGRectMake(X0, Y0, 1.3*(X1-X0), 1.3*(Y1-Y0))
